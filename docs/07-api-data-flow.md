...
| **Database** | **HAI** truy vấn, cùng đi qua server client chịu RLS (PHASE 17 — **DEC-068**):<br>1. `getReportForShare()` — một dòng, join `profiles` lấy `full_name`, `employee_code`; nay kéo thêm **`sales_id`** để biết cộng lũy kế cho ai.<br>2. `listMonthToDateMetrics()` — tối đa **31 dòng × 8 cột số** trong khoảng `[ngày 01 của tháng báo cáo, mốc dừng]`, bám `idx_daily_reports_sales_date_desc`. **Bỏ qua hoàn toàn** khi khoảng rỗng (ảnh sáng của ngày 01).<br>Chọn đúng cột cần, không `select *` |

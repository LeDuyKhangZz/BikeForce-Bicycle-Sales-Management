...
| **Số liệu AMIS bổ sung** | `no_of_orders`, `current_amount / no_of_orders` và `return_sales` được dựng thành dải **SỐ ĐƠN · TB / ĐƠN · HÀNG TRẢ LẠI**. Chúng không có target thật nên không vào `KpiMetric`, không tính `%` và không tham gia BR-024. Mẫu số 0 hoặc dữ liệu không dùng được → `—`, tuyệt đối không `NaN`/`Infinity` |

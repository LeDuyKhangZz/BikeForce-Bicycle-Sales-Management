...
| `e2e/share-image.spec.ts` | **MỚI (DEC-060)** — **bấm thật** nút xuất ảnh: bắt sự kiện `download` và kiểm tên file FR-019 · máy tính **KHÔNG** mở share sheet · thiết bị cảm ứng gửi đúng file `image/png` > 1KB vào share sheet · link "Mở ảnh trực tiếp" tồn tại và trỏ đúng route |
...
### 2026-09-13 — Hồi quy retry UPSERT AMIS

`python -m unittest discover -s scripts/amis-sync -p 'test_push_amis_*.py'`
đã chạy 12/12 pass. Test mới tái hiện ConnectionResetError 10054 trước fix,
...
DEC-098 yêu cầu RLS test tối thiểu: Sales A INSERT/UPDATE khách của mình thành công; Sales A INSERT/UPDATE khách của Sales B bị từ chối; Sales inactive bị từ chối; Admin vẫn INSERT/UPDATE được. E2E Sales phải nhập trực tiếp theo khoảng dòng và lưu toàn bộ. Unit validation hiện có tiếp tục khóa giới hạn 0–31, tiền không âm và tối đa 2.000 dòng.

### 2026-09-25 — Workbook kế hoạch XLSX

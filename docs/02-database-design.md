...
Migration `20260912110000_monthly_participant_salaries.sql`: bảng lương participant PK `(period_month, participant_key)`, bigint nullable không âm, tháng ngày 01, allowlist ba participant, audit `updated_by/updated_at`; RLS enabled + forced, chỉ Admin active SELECT/INSERT/UPDATE, không DELETE/service_role. RPC `save_monthly_salary_entries` SECURITY INVOKER lưu nguyên tử bảng cũ cho Sales UUID và bảng mới cho participant; xác thực Admin, kiểm tra người nhận. Không đổi bảng/RLS/lương Sales cũ.
...
| `visit_purpose` | `text` | YES | `null` | `ck_visit_purpose_len`: `visit_purpose is null or char_length(visit_purpose) <= 300` | ⚠ **DI SẢN từ 2026-08-10 (DEC-048).** Không còn được ghi và không còn hiển thị ở màn hình nào. Cột ở lại cùng dữ liệu đã nhập vì **BR-013 cấm xoá**. |
| `target_visit_points` | `integer` | NO | — | `ck_target_visit_points`: **`between 5 and 1000`** (`20260918090000`, `not valid`) | Số điểm/đại lý **mục tiêu**. **Sàn 5 từ 2026-09-18 — BR-026, DEC-091.** Sàn chỉ áp cho `target`. |
| `target_sales_quantity` | `integer` | **YES** *(bỏ NOT NULL ở `0008`)* | `null` | `ck_target_sales_quantity`: `between 0 and 10000` | ⚠ **DI SẢN từ 2026-08-10 (DEC-050).** Nghĩa CŨ: số lượng xe cam kết. Không còn được ghi. Chỉ tiêu "Doanh số" nay đọc `target_sales_amount`. |
| **`target_sales_amount`** | `bigint` | YES *(bắt buộc qua `ck_target_sales_amount_required`, `not valid`)* | `null` | `ck_target_sales_amount`: `is null or between 0 and 100000000000` | **MỚI ở `0008` (DEC-050).** Doanh số bán hàng cam kết trong ngày, **VND nguyên** (BR-010). `null` **chỉ** ở các dòng có trước 2026-08-10 (OQ-19c). |
| `target_revenue` | `bigint` | NO | — | `ck_target_revenue`: `between 0 and 100000000000` | Doanh thu mục tiêu, **VND nguyên** (BR-010). ⚠ **Nghĩa đổi từ 2026-08-10 (DEC-050):** nay là **công nợ khách hàng dự kiến THU HỒI** trong ngày. Dòng trước mốc đó mang nghĩa cũ "giá trị đơn hàng chốt trong ngày". |
...
| `actual_sales_quantity` | `integer` | YES | `null` | `ck_actual_sales_quantity`: `... between 0 and 10000` | ⚠ **DI SẢN từ 2026-08-10 (DEC-050).** Nghĩa CŨ: số lượng xe thực bán. Không còn được ghi. |
| **`actual_sales_amount`** | `bigint` | YES | `null` | `ck_actual_sales_amount`: `is null or between 0 and 100000000000` | **MỚI ở `0008` (DEC-050).** Doanh số bán hàng **thực đạt**, VND nguyên. Nằm trong `ck_completed_requires_actuals` thay cho cột di sản. |
| `actual_revenue` | `bigint` | YES | `null` | `ck_actual_revenue`: `... between 0 and 100000000000` | ⚠ **Nghĩa đổi từ 2026-08-10 (DEC-050):** công nợ khách hàng **thực THU HỒI** trong ngày, VND nguyên. Dòng trước mốc đó mang nghĩa cũ (OQ-14). |

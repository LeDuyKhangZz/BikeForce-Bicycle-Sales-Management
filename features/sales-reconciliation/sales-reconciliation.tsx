  ...
  return (
    <div className="flex items-baseline justify-between gap-4">
      ...
      <span className="tabular-nums text-sm font-medium text-slate-900">
        {value === null ? <span className="text-slate-400">—</span> : formatCurrencyVND(value)}
      </span>
    </div>
  );
}
        ...
        {row.revenue_diff === null ? (
          ...
          <div className="flex items-baseline justify-between gap-4">
            ...
            <span
              className={
                level === "warning"
                  ? "tabular-nums text-sm font-semibold text-red-600"
                  : "tabular-nums text-sm font-medium text-emerald-700"
              }
            >
              {row.revenue_diff > 0 ? "+" : ""}

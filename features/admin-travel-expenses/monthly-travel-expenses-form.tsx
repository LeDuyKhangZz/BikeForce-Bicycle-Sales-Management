...
import { travelExpenseFieldName } from '@/lib/validation/travel-expenses';

import { saveTravelExpensesAction, type SaveTravelExpensesState } from './actions';

export type TravelExpenseSalesRow = {
  id: string;
  full_name: string;
...
type Props = {
  month: string;
  monthLabel: string;
  salesList: readonly TravelExpenseSalesRow[];
...
export function MonthlyTravelExpensesForm({
  month,
  monthLabel,
  salesList,
  currentAmounts,
}: Props) {
  const [state, formAction, isPending] = useActionState<SaveTravelExpensesState, FormData>(
    saveTravelExpensesAction,
    null,
  );
  ...
  return (
    <form action={formAction} className="flex flex-col gap-4" noValidate>
            ...
            <li key={sales.id}>
              <Card className="flex h-full flex-col gap-3">
                ...
                <div className="flex flex-col gap-2">
                  <Label htmlFor={field}>
                    Công tác phí <span className="sr-only">của {sales.full_name}</span>
                  </Label>
                  <Input
                    id={field}
                    name={field}
      ...
      <Button type="submit" size="lg" loading={isPending} loadingText="Đang lưu công tác phí…">
        <Save aria-hidden="true" className="size-5" />
        Lưu công tác phí {monthLabel}
      </Button>
    </form>
  );
}

...
vi.mock('@/services/salaries', () => ({ listSalaryEntries: vi.fn() }));
const client = createClient<Database>('http://127.0.0.1:54321', 'unit-test-key');
beforeEach(() => vi.resetAllMocks());

it('danh sách nhập lương và danh sách báo cáo dùng cùng khóa, đọc lại lương theo tháng', async () => {
  vi.mocked(listSalesOptions).mockResolvedValue([]);
  vi.mocked(listSalaryEntries).mockResolvedValue([
  ...
  const result = await getSalaryPageData(client, '2026-08-01');
  ...
  expect(listSalaryEntries).toHaveBeenCalledWith(client, '2026-08-01');
...
it('Khoa đã có profile Sales dùng UUID cũ, không thêm dòng hoặc chuyển lương sang khóa tích hợp', async () => {
  vi.mocked(listSalesOptions).mockResolvedValue([
    { id: 'existing-khoa', full_name: 'Nguyễn Trần Đăng Khoa', employee_code: 'VP-PTM-001', is_active: true },
  ]);
  vi.mocked(listSalaryEntries).mockResolvedValue([{ sales_id: 'existing-khoa', amount: 20000000 }]);
  const result = await getSalaryPageData(client, '2026-09-01');

...
export const travelExpenseMonthSchema = z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/);
...
export const travelExpenseInputSchema = z
  ...
  .transform((raw, context): number | null => {
    ...
    if (typeof raw !== 'string') {
      context.addIssue({ code: 'custom', message: 'Chỉ nhập số tiền, ví dụ 3.000.000.' });
      return z.NEVER;
    }
...
export function travelExpenseFieldName(salesId: string): string {

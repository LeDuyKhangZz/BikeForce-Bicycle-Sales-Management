...
test('HTTP-only: lưu form công tác phí thật, mở lại và lấy ảnh cả ba người đúng tháng', async ({ request }) => {
  ...
  const path = '/admin/travel-expenses?month=2043-06';
  ...
  expect(await saved.text()).toContain('Đã lưu công tác phí tháng.');
  const reopened = await request.get(path, { headers: { Cookie: cookieHeader } });
  const reopenedHtml = await reopened.text();
  const payroll = await request.get(`${env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/monthly_participant_travel_expenses?select=participant_key,amount&period_month=eq.2043-06-01`, {
  ...
  for (const [id, amount] of Object.entries(amounts)) {
    ...
    const image = await request.get(`/api/admin/monthly-summaries/${id}/image?month=2043-06`, { headers: { Cookie: cookieHeader } });

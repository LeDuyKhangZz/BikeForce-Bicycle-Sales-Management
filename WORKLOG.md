...
Theo đính chính của người dùng, ảnh telesale (không phải Tổng kết tháng) dùng `receive_amount` cho cột THỰC ĐẠT của dòng “Doanh thu đã ghi”; Nguyễn Thị Như Quỳnh hiện là 721.709.926. Không đổi dòng doanh số hoặc báo cáo tháng. Unit card 3/3, typecheck/lint pass.

Sau khi Vercel lỗi vì trang Nhân viên còn gọi token CRM, đã chuyển danh sách/chi tiết sang đọc snapshot Supabase bằng session Admin chịu RLS. Tìm kiếm, toàn bộ toán tử lọc và phân trang chạy server-side; request web không còn phụ thuộc phiên MISA. CLI không generate được types do tài khoản linked thiếu quyền endpoint; không sửa tay file generated, service kiểm tra runtime response.
